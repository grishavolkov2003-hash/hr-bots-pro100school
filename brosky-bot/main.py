...
async def _process_qa_message(user_id, username, name, chat_id):
    ...
    if not response:
        if llm_failed:
            ...
            await _send_message_safe("mx_mish", f"⚠️ QA LLM не ответил {name} ({username}) после повтора - нужно ответить вручную.\nПоследнее сообщение: {last_text}")
        return
...
async def _do_process(user_id, username, name, chat_id, candidate):
    ...
    if statuses:
        ...
        # ТЕСТОВОЕ_ПОЛУЧЕНО — как и в hr-bot, только код ставит при реальном видео
...
async def _patrol_respond(user_id, username, name, delay):
    ...
    try:
        ...
        if statuses:
            llm_status = statuses[-1].strip()
            if llm_status in VALID_STATUSES and llm_status != "ТЕСТОВОЕ_ПОЛУЧЕНО" and llm_status != old_status and old_status != "ПЕРЕДАН_МЕНЕДЖЕРУ":
