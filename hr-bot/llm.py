...
client = anthropic.Anthropic(
    ...
    base_url=BASE_URL,
    # Таймаут 45с вместо дефолтных десятков минут httpx-клиента - защита от
    # зависшего origin на стороне прокси-провайдера (независимо от того,
    # какой именно провайдер сейчас используется). Без своего таймаута
    # кандидат мог бы провисеть по несколько минут на каждой из двух
    # попыток в _call_llm, прежде чем система сдастся и пошлёт alert.
    timeout=45.0,
    # SDK по умолчанию сам молча повторяет упавший запрос (max_retries=2)
    # ДО того как отдать исключение нашему коду - на практике это раздувало
    # интервал между "попытка 1/2" и "попытка 2/2" в логах до ~140с вместо
    # ожидаемых ~48с (45 + 3), то есть таймаут выше держал не то, что должен.
    # У _call_llm уже есть свой явный и залогированный retry - отключаем
    # скрытый, чтобы таймаут реально ограничивал одну попытку 45 секундами.
    max_retries=0,
)
